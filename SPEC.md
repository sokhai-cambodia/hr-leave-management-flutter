...
- **Dashboard**: role-adaptive summary — leave balances, pending requests, recent activity, entry point to AI recommendations; approvals count badge for team owners. Quick Actions grid intentionally omits Profile (reachable only via the drawer's "Settings / Profile" entry, to avoid a duplicate nav surface — see `tasks/plan.md` Task 10.2). "Pending Requests" and "Recent Activity" stat cards are still placeholders (`dashboard_view.dart`'s `_PlaceholderStatCard`), not wired to real data — a known, not-yet-scoped gap.
...
- **Public Holidays (employee view)**: month-view calendar (`table_calendar` package) highlighting holiday dates with month-by-month navigation, plus a list of the visible month's holidays underneath. No server-side month filter exists (`GET /public-holidays/` only supports `skip`/`limit`), so the client fetches once and groups/filters locally — see `tasks/plan.md` Task 10.1 for the reusable grouping/filtering logic shape (relevant for the Android/Compose equivalent, §9).
