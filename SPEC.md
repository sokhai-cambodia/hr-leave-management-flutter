...
- **Dashboard**: role-adaptive summary — leave balances, pending requests (employee's own pending submissions), pending approvals count for team owners (tappable, opens the Approvals queue), entry point to AI recommendations. Quick Actions grid intentionally omits Profile (reachable only via the drawer's "Settings / Profile" entry, to avoid a duplicate nav surface — see `tasks/plan.md` Task 10.2). The "Recent Activity" placeholder stat card was removed (Task 10.3) rather than left permanently fake.
