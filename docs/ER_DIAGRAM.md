...
- **`LeaveRequest`/`LeavePlanRequest.approver_id`** is nullable and only populated once a request is submitted (an employee with no team, or a team with no `team_owner_id`, can have a request stuck without an approver — surfaced client-side as a friendly error).
