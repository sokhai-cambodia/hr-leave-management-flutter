...
- **Acceptance:** all three — `flutter analyze` clean, `flutter test` green (only the pre-existing unrelated `dio_client_unauthorized_test.dart` failures remain), backend confirmed live via Render's OpenAPI schema (`/api/v1/openapi.json` lists `/approvals/pending-count` and `/schedule/` with the expected params) since local backend verification wasn't possible (no C++ build toolchain for `uv sync`, no local `.env`/Docker set up this session).
- **Verify:** manual — Dashboard shows a real "Pending Approvals" count for a team-owner test account, tapping it opens Approvals; Schedule screen (dashboard tile + drawer) shows holiday markers and team-leave markers with distinct colors, month navigation re-fetches correctly, list sections match.
